...
def render_additive_tab():
    ui.label('Árbol Aditivo (Método Single-Linkage)').classes('text-2xl font-bold mb-6')
    ...
    def generar():
        try:
            figura = generar_arbol(entrada.value, metodo='single')
            ui.pyplot(figure=figura)
            resultado.text = 'Árbol generado correctamente.'
        except Exception as e:
            resultado.text = f'Error: {e}'

    ui.button('GENERAR ÁRBOL ADITIVO', on_click=generar).classes('mt-4')
